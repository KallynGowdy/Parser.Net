...
namespace KallynGowdy.ParserGenerator.Collections
{
		...
		where TColumn : IEquatable<TColumn>
	{
		...
		private KeyedDictionary<TRow, KeyedDictionary<TColumn, TValue>> lookup;
		...
		/// </summary>
		public IEqualityComparer<TColumn> ColumnComparer { get; private set; }
		...
		public TValue this[TRow row, TColumn column]
		{
			...
			{
				TValue result;
				TryGetValue(row, column, out result);
				return result;
			}
			set { lookup[row][column] = value; }
		}
		...
		{
			get { return lookup.Values.SelectMany(a => a.Values).ToArray(); }
		}
		...
		{
			KeyValuePair<TRow, KeyedDictionary<TColumn, TValue>>[] temp = lookup.ToArray();
