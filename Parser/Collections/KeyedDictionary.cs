...
namespace KallynGowdy.ParserGenerator.Collections
{
	...
	{
		private readonly Dictionary<TKey, Tuple<TKey, TValue>> lookup;

		/// <summary>
		///     Creates a new dictionary that supports retrival of keys that may contain different information than the key used to
		///     access it.
		/// </summary>
		...
		{
			get { return lookup.Values.Select(a => a.Item2).ToArray(); }
		}
		...
		{
			get { return lookup[key].Item2; }
			set { lookup[key] = new Tuple<TKey, TValue>(key, value); }
		}
		...
		{
			KeyValuePair<TKey, Tuple<TKey, TValue>>[] temp = lookup.ToArray();
