...
namespace KallynGowdy.ParserGenerator.Parsers
{
	...
	public class ParseTableConflict<T>
		where T : IEquatable<T>
	{
		...
		/// </summary>
		public ReadOnlyCollection<LRItem<T>> ConflictingItems { get; private set; }

		/// <summary>
		///     Gets the type of the conflict(either Shift_Reduce or Reduce_Reduce)
		/// </summary>
		public ParseTableExceptionType ConflictType { get; private set; }

		/// <summary>
		///     Gets the StateNode where the conflict takes place in the graph.
		/// </summary>
		public StateNode<GrammarElement<T>, LRItem<T>[]> GraphLocation { get; private set; }

		/// <summary>
		///     Gets the location of the conflict in the parse table.
		/// </summary>
		public ColumnRowPair<int, Terminal<T>> TableLocation { get; private set; }
	}
}