...
namespace Parser.Parsers
{
    ...
    public class LRParser<T> : IGrammarParser<T>, IGraphParser<T>
    {
        ...
        public ParseTree<T> ParseAST(IEnumerable<Terminal<T>> input)
        {
            ...
            foreach (Terminal<T> item in augmentedInput)
            
            {
                int s = stateStack.Peek().Key;
                ParserAction<T> action = ParseTable.ActionTable[s, item].First();
                ...
                if (action != null)
                {
                    ...
                    else if (action is ReduceAction<T>)
                    {
                        ...
                        for (int i = 0; i < r.ReduceItem.ProductionElements.Length; i++)
                        {
                        ...
                        {
                            ParseTree<GrammarElement<T>>.ParseTreebranch newBranch = new ParseTree<T>.ParseTreebranch(r.ReduceItem.LeftHandSide);
                        ...
                        stateStack.Push(new KeyValuePair<int, GrammarElement<T>>(parseTable.GotoTable[s, r.ReduceItem.LeftHandSide].Value, r.ReduceItem.LeftHandSide));
                    }
