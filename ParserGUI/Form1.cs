...
namespace ParserGUI
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                //AccessMod -> public | private | protected | internal | protected internal | internal protected | nothing
                var accessModForm = a.ZeroOrOne.OneOf
                (b =>
                    new GrammarElement<string>[]
                    {
                        "public".ToTerminal(),
                        "private".ToTerminal(),
                        "protected".ToTerminal(),
                        "internal".ToTerminal(),
                        b.Form
                        (() =>
                           new[] { "protected".ToTerminal(), "internal".ToTerminal() }
                        ),
                        b.Form
                        (() =>
                            new[] { "internal".ToTerminal(), "protected".ToTerminal() }
                        )
                    }
                );

                //Method -> accessMod v a ( ) { }
                return a.Form
                (
                    accessModForm, "v".ToTerminal(), "a".ToTerminal(), "(".ToTerminal(), ")".ToTerminal(), "{".ToTerminal(), "}".ToTerminal() 
                );
            });
        ...
        {
            MessageBox.Show(parser.ParseAST(txtCFG.Text.Select(a => new Terminal<string>(a.ToString()))).Success.ToString());
        }
    }
}
